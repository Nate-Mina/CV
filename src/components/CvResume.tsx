...
import { TechnicalSkillsVisualizer } from './TechnicalSkillsVisualizer';
...
export const CvResume: React.FC<CvResumeProps> = ({
  currentTheme,
  onOpenTerminal,
  onOpenGallery,
  onNavigateToTab,
  ...
  const [selectedClinicalDomain, setSelectedClinicalDomain] = useState<string>('dsm5-psychology');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-12">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const el = document.getElementById('technical-skills-visualizer');
                    el?.scrollIntoView({ behavior: 'smooth' });
                  }}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-bold transition-all shadow-sm"
                  title="Jump to Quantitative Skills Radar & Heatmap"
                >
                  <Cpu className="w-3.5 h-3.5 text-cyan-400" />
                  <span>Skills Radar &amp; Heatmap</span>
                </button>

                <button
                  type="button"
                  onClick={() => {
                    const el = document.getElementById('clinical-supplement-medicine-dsm5');
                    el?.scrollIntoView({ behavior: 'smooth' });
                  }}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-mono font-bold transition-all shadow-sm"
                  title="Jump to Clinical Medicine, Pharmacology & DSM-5 Psychology"
                >
                  <HeartPulse className="w-3.5 h-3.5 text-rose-400" />
                  <span>Clinical &amp; DSM-5</span>
                </button>
              </div>
      ...
      {/* Quantitative Technical Skills Radar & Heatmap Grid */}
      <TechnicalSkillsVisualizer
        currentTheme={currentTheme}
        onNavigateToCaseStudies={() => onNavigateToTab && onNavigateToTab('cases')}
      />

      {/* Dedicated Section: Psychological Science (DSM-5), Pharmacology & Internal Medicine */}
      <div
        id="clinical-supplement-medicine-dsm5"
        className={`p-6 sm:p-8 rounded-2xl border ${currentTheme.borderClass} ${currentTheme.cardBgClass} shadow-2xl space-y-6`}
      >
        {/* Top Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-rose-400">
              ...
              <span>Biopsychosocial &amp; Evidence-Based Analytics</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-mono font-bold text-slate-100 flex items-center gap-2">
              Psychological Science (DSM-5), Pharmacology &amp; Internal Medicine
            </h2>
            <p className="text-xs sm:text-sm font-sans text-slate-300 leading-relaxed max-w-2xl">
              Cross-disciplinary clinical synthesis integrating APA DSM-5 diagnostic frameworks,
              pharmacokinetics (ADME), endothelial and endocrine physiology, and the VitalStats Clinical Supplement Analytics Portal.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <a
              href="./supplement_portal.html"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-mono font-bold transition-all shadow-sm"
              title="Launch VitalStats Clinical Evidence Portal in New Window"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              <span>Launch VitalStats Portal</span>
            </a>
            {onNavigateToTab && (
              <button
                type="button"
                onClick={() => onNavigateToTab('cases')}
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700 text-xs font-mono transition-all"
              >
        ...
        {/* Clinical Synthesis Metrics Bar */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center space-y-1">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider">Surveillance Population</div>
            <div className="text-xl font-bold text-rose-400">57.6%</div>
            <div className="text-[11px] text-slate-400">CDC NHANES Cohort</div>
          </div>
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center space-y-1">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider">SUCRA Synthesis</div>
            <div className="text-xl font-bold text-emerald-400">97.0%</div>
            <div className="text-[11px] text-slate-400">Multi-Arm Synergy Peak</div>
          </div>
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center space-y-1">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider">Clinical Compounds</div>
            <div className="text-xl font-bold text-cyan-400">15+</div>
            <div className="text-[11px] text-slate-400">Systematically Synthesized</div>
          </div>
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center space-y-1">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider">Adulteration Audits</div>
            <div className="text-xl font-bold text-amber-400">776+</div>
            <div className="text-[11px] text-slate-400">FDA Recalls Cataloged</div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2 border-b border-slate-800/80 pb-3">
            {profileData.clinicalKnowledge?.map((domain) => {
              const isSelected = selectedClinicalDomain === domain.id;
              return (
                <button
                  key={domain.id}
                  type="button"
                  onClick={() => setSelectedClinicalDomain(domain.id)}
                ...
                >
                  {domain.id === 'dsm5-psychology' && <Brain className="w-3.5 h-3.5 text-rose-400" />}
                  {domain.id === 'medicines-pharmacology' && <Pill className="w-3.5 h-3.5 text-pink-400" />}
                  {domain.id === 'internal-medicine-endothelial' && <Activity className="w-3.5 h-3.5 text-rose-400" />}
                  {domain.id === 'vitalstats-synthesis' && <FlaskConical className="w-3.5 h-3.5 text-emerald-400" />}
          ...
          {/* Active Domain Detailed Presentation */}
          {(() => {
            const activeDomain = profileData.clinicalKnowledge?.find(
              (d) => d.id === selectedClinicalDomain
            ) || profileData.clinicalKnowledge?.[0];

            if (!activeDomain) return null;

            return (
              <div className="p-5 sm:p-6 rounded-xl bg-slate-950/80 border border-slate-800 space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                  ...
                  <span className="self-start sm:self-center text-[11px] font-mono px-2.5 py-1 rounded bg-rose-500/10 text-rose-300 border border-rose-500/30 whitespace-nowrap">
                    {activeDomain.caseStudyRef}
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {/* Core Theoretical & Diagnostic Foundations */}
                  <div className="space-y-2.5 p-4 rounded-xl bg-slate-900/50 border border-slate-800/80">
                    <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-rose-400 font-bold">
                      <BookOpen className="w-4 h-4" />
                      <span>Theoretical &amp; Diagnostic Foundations</span>
                    </div>
                    <ul className="space-y-2 text-xs font-sans text-slate-300">
                      {activeDomain.coreConcepts.map((concept, cIdx) => (
                  ...
                  {/* Applied Clinical Engineering & Synthesis */}
                  <div className="space-y-2.5 p-4 rounded-xl bg-slate-900/50 border border-slate-800/80">
                    <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-emerald-400 font-bold">
                      <ShieldCheck className="w-4 h-4" />
                      <span>Applied Engineering &amp; Clinical Utility</span>
                    </div>
                    <ul className="space-y-2 text-xs font-sans text-slate-300">
                      {activeDomain.keyApplications.map((app, aIdx) => (
                ...
                {/* Direct VitalStats Portal Callout Banner */}
                <div className="p-4 rounded-xl bg-gradient-to-r from-rose-950/30 via-slate-900/80 to-emerald-950/30 border border-rose-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="text-xs font-mono font-bold text-slate-200 flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-rose-400" />
                      <span>VitalStats | Clinical Supplement Analytics &amp; Synthesis Portal (v4.2)</span>
                    </div>
                    <p className="text-xs font-sans text-slate-300">
                      Explore the live interactive database featuring 15+ compound dossiers, contraindication algorithms, and NHANES cohort trends.
                    </p>
                  </div>
                  <a
                    href="./supplement_portal.html"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-rose-500 hover:bg-rose-600 text-white font-mono text-xs font-bold transition-all shadow-lg shadow-rose-950/50 shrink-0"
                  >
